...
const CAPABILITIES = [
  ...
  ['호스팅 러너 실험실에서 서비스 시작과 프롬프트 감시 헬퍼 실행', '일부 확인', '소프트웨어 키 공급자를 쓰는 실험실 빌드에서 서비스가 실행 상태에 도달하고 감시 헬퍼가 대화형 세션에서 실행되는 것까지 확인했습니다. 헬퍼가 실제 UAC 창을 관측한 기록은 아직 없습니다.'],
  ...
  ['TPM 기반 서비스 키 시작', '알려진 문제', '이전 판의 서비스는 쓰기 제한 토큰 때문에 키를 저장하지 못했고, 개발 PC에서는 0x80090030으로 시작에 실패한 기록이 있습니다. 이 판은 서비스 등록 방식을 바꿨지만 실제 PC에서 다시 확인한 기록은 아직 없습니다.'],
];
