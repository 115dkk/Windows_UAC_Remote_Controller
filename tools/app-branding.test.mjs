...
test('PC, Android and client share the explicit UAC app name and source logo', () => {
  ...
  assert.equal(config.productName, 'UAC 원격 승인기');
  ...
  assert.equal(JSON.parse(text('locales/ko.json'))['UAC 원격 승인'], 'UAC 원격 승인기');
  ...
  assert.match(text('ui/index.html'), /<title>UAC 원격 승인기<\/title>/u);
