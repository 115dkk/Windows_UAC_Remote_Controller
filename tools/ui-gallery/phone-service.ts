...
export function registerPhoneServiceGallery(test: typeof GalleryTest): void {
  for (const selected of phoneServiceGalleryCases) {
    test(selected.id, async ({ page, gallery }) => {
      ...
      const panel = page.getByRole('region', { name: '휴대폰 서비스', exact: true });
      ...
      if (ready) {
        await expect(panel.getByText('서비스가 실행 중이에요', { exact: true })).toBeVisible();
      ...
      if (selected.fixture === 'phone-service-stopped') {
        await expect(panel.getByText('서비스가 중지되어 있어요', { exact: true })).toBeVisible();
        await expect(panel.getByText('꺼짐', { exact: true })).toBeVisible();
        await expect(panel.getByRole('button', { name: '서비스 시작', exact: true })).toBeEnabled();
        await expect(panel.getByRole('button', { name: '서비스 중지', exact: true })).toHaveCount(0);
      }
      if (selected.fixture === 'phone-service-preparing') await expect(panel.getByText('서비스를 준비하고 있어요', { exact: true })).toBeVisible();
      ...
      if (selected.fixture === 'phone-service-error') {
        await expect(page.getByRole('alert')).toContainText('휴대폰 서비스를 시작하지 못했어요.');
      ...
      await gallery.capture('overview', '휴대폰 서비스 상태 · 합성 클라이언트 화면, 실제 서비스 아님');

      if (selected.fixture === 'phone-service-stopped') {
        const start = panel.getByRole('button', { name: '서비스 시작', exact: true });
        await start.scrollIntoViewIfNeeded();
        await start.focus();
        await expect(start).toBeFocused();
        await page.keyboard.press('Enter');
        await expect(panel.getByText('서비스를 준비하고 있어요', { exact: true })).toBeVisible();
        await expect(panel.getByText('켜짐', { exact: true })).toBeVisible();
        await expect(panel.getByRole('button', { name: '서비스 시작', exact: true })).toHaveCount(0);
        await expect(page.getByRole('radio')).toHaveCount(0);
        await expect(panel.getByText('서비스가 실행 중이에요', { exact: true })).toHaveCount(0);
        await gallery.capture('start-request-simulated', '합성 시작 응답은 준비 중일 뿐 실행 완료가 아님');
      }
      if (ready) {
        const stop = panel.getByRole('button', { name: '서비스 중지', exact: true });
        await stop.scrollIntoViewIfNeeded();
        await stop.focus();
        await page.keyboard.press('Enter');
        let dialog = page.getByRole('dialog', { name: '휴대폰 서비스를 중지할까요?', exact: true });
        ...
        await page.keyboard.press('Enter');
        dialog = page.getByRole('dialog', { name: '휴대폰 서비스를 중지할까요?', exact: true });
        await dialog.getByRole('button', { name: '서비스 중지', exact: true }).click();
        await expect(panel.getByText('이전 작업을 정리하고 있어요', { exact: true })).toBeVisible();
        await expect(panel.getByText('꺼짐', { exact: true })).toBeVisible();
        await expect(panel.getByRole('button')).toHaveCount(0);
        await expect(page.getByRole('radio')).toHaveCount(0);
