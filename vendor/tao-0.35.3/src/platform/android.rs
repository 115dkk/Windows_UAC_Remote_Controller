...
pub use crate::platform_impl::activity::{
  observe_existing_activities, ExistingActivity, ExistingActivityError, ExistingActivityEvent,
  ExistingActivityObserver,
};
use crate::{
...
pub trait WindowExtAndroid {
  /// Original physical incarnation; configuration recreation never rebinds it.
  fn existing_activity(&self) -> ExistingActivity;
...
pub trait WindowBuilderExtAndroid {
  /// Attach exactly once to this registered instance; never launch/fall back.
  fn with_existing_activity(self, activity: ExistingActivity) -> Self;
...
impl WindowBuilderExtAndroid for WindowBuilder {
  fn with_existing_activity(mut self, activity: ExistingActivity) -> Self {
    self.platform_specific.existing_activity = Some(activity);
    self
  }
