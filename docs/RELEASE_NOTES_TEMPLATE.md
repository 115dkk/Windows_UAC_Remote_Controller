...
- PC·Android 앱 이름과 자체 승인 로고를 통일했습니다.
- PC의 휴대폰 관리와 QR 표시 버튼을 항상 찾을 수 있게 하고, 준비 전에는 필요한 단계를 안내합니다.
- 미연결 상태와 빈 요청 목록을 구분하고 사용할 수 없는 항목을 흐리게 표시합니다.
- 양쪽의 QR 연결 안내를 맞추고 기존 Windows 설치 경로·연결 정보는 유지합니다.
