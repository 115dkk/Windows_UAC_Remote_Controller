...
#### Indexed origins and local search priority only
...
`RevisionOrigin(revision, pc, device)` is produced only in the existing TrustedEnrollment, TrustedReenrollmentWithSameKeys and TrustedReplacementWithSameKeys rules, from their existing fresh revision/device values, immediately before the corresponding new RegistrySlot output. It persists as provenance after revocation, **not current membership**. Each RegistrySlot input is immediately preceded by the matching RevisionOrigin premise, including `old_revision` for re-enrollment/replacement. Every exact active/retired RegistrySlot check remains mandatory; old provenance cannot authorize a new revision or replace a frozen Snapshot.

Only RequestOrigin and RevisionOrigin **input premises** carry `[+]`; all outputs and other facts are unannotated. The [pinned fact-annotation manual](https://github.com/tamarin-prover/tamarin-prover/blob/1.12.0/manual/src/011_advanced-features.md#sec:fact-annotations) defines this as local heuristic priority, not a unification change. It is intended to solve the short origin join before recursive state-history constraints. No `no_precomp`, new theorem/assumption, trace restriction, bound, rule action or phone-input condition is introduced; neither detection nor improved convergence is claimed.

The inverse for this latest step is exact at the intended rule level: erase all RevisionOrigin facts and origin `[+]` annotations, then remove the added first argument of RequestOrigin. This recovers the previous slot-based rules. Conversely, on every previous reachable state the request ID was already fixed by its binding, and every active/retired revision slot already descended from one of those three fresh-creation rules; its matching persistent origin can therefore be carried without removing a trace. This correspondence concerns the current overapproximating model, not an implementation proof. ROOT must check inverse-erasure and unchanged all15 rules/all9 lemma headers/formulas, public messages, actions, FULL Snapshot, EnrollmentKeys and signing domains. Manifest/canary text remains unchanged: the replay mutant still restores the same global pending RequestSlot and must expose the real two-accept attack.
