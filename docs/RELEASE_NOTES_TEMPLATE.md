...
- Android 배포 APK의 UniFFI/JNA 네이티브 인터페이스를 난독화 대상에서 보존합니다.
- PC 연결 전 초기화, QR 촬영 진입과 알림 설정을 실제 난독화된 release APK로 검사하는 CI를 추가했습니다.
- 알림 설정을 읽기 전 앱 준비 단계에서 발생한 문제를 ‘알림 시간 설정 읽기 실패’와 구분합니다.
