...
impl ToTokens for CachedIcon {
  fn to_tokens(&self, tokens: &mut TokenStream) {
    ...
    // An explicit array length gives rust-analyzer's include_bytes expansion
    // the same type as rustc. The cached bytes are unchanged; rustc still
    // checks their actual length and rejects any mismatch at compile time.
    let raw = quote!({
      const BYTES: &[u8; #byte_len] = ::std::include_bytes!(#cache);
      BYTES
    });
