...
impl Reporter {
    ...
    fn report(
        ...
        // Repeated interrogation/progress may expose only a report that SCM
        // actually accepted, never a speculative readiness state.
        self.last = status;
        self.phase = phase;
        Ok(())
    }
        ...
        if finished.is_none() {
            match worker.next_event()? {
                Some(WorkerEvent::Progress) if !stopping => {
                    ...
                    // Once early Running succeeded, later initialization progress
                    // repeats that no-controls state; it cannot regress to Starting.
                ...
                Some(WorkerEvent::ScmRunningRequired(request)) if !stopping => {
                    ...
                    // This only releases platform initialization. Do not clear
                    // pending_since or enable any probe/control admission here.
                }
                Some(WorkerEvent::Ready) if !stopping => {
                    remaining_budget(startup_began.elapsed())?;
                    reporter.report(reporter.phase.ready()?, 0, None)?;
