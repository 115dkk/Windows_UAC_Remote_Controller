...
import { DEFAULT_ANDROID_ABI, androidElfIdentity, selectAndroidAbi } from './android-abi.mjs';

export const EXPECTED_LIBRARIES = expectedLibrariesForAbi();
export function expectedLibrariesForAbi(abi = DEFAULT_ANDROID_ABI) {
  const selected = selectedAbi(abi);
  return Object.freeze(['libcontroller_app_lib.so', 'libuac_android_controller.so', 'libjnidispatch.so']
    .map((name) => `lib/${selected.abi}/${name}`));
}
...
function selectedAbi(abi) {
  try { return selectAndroidAbi(abi); }
  catch { reject('APK ABI must be arm64-v8a or x86_64.'); }
}

/** ELF program-header shape only, not relocation/symbol/runtime compatibility. */
export function checkElf64Android(bytes, abi = DEFAULT_ANDROID_ABI) {
  const selected = selectedAbi(abi);
  ...
  try { androidElfIdentity(bytes, selected.abi); }
  catch { reject('Selected library is not an ELF64 little-endian shared object for the selected Android ABI.'); }
  ...
  return { format: 'ELF64LE', type: 'ET_DYN', machine: selected.elfMachine, architecture: selected.elfArchitecture,
    bytes: bytes.length, programHeaderCount: entryCount, loadSegments: loads };
}

/** Existing arm64-only API remains an arm64 check, never infers ABI from bytes. */
export function checkElf64Arm64(bytes) { return checkElf64Android(bytes, DEFAULT_ANDROID_ABI); }

/** Parses unzip's filename-only listing, not ZIP binary structures. */
export function inspectArchiveListing(bytes, abi = DEFAULT_ANDROID_ABI) {
  const selected = selectedAbi(abi);
  const required = expectedLibrariesForAbi(selected.abi);
  ...
  for (const name of names) {
    ...
    if (parts[1] !== selected.abi) reject('APK advertises an ABI other than the selected Android ABI.');
    if (name === `lib/${selected.abi}/`) continue;
  ...
  if (required.some((name) => !seen.has(name))) reject('APK is missing a required native library for the selected Android ABI.');
  return { entryCount: names.length, advertisedAbis: [selected.abi], nativeLibraryCount,
    additionalSameAbiLibraries: nativeLibraryCount - required.length,
    ...(selected.abi === DEFAULT_ANDROID_ABI ? { arm64LibraryCount: nativeLibraryCount,
      additionalArm64Libraries: nativeLibraryCount - required.length } : {}) };
}

export function apkSelectionFromArguments(args, cwd = process.cwd(), platform = process.platform) {
  if (platform !== 'linux') reject('APK inspection CLI supports the CI Linux runner only.');
  if (!Array.isArray(args) || typeof args[0] !== 'string' ||
      (args.length !== 1 && (args.length !== 3 || args[1] !== '--abi' || typeof args[2] !== 'string'))) {
    reject('Usage: node tools/verify-android-apk.mjs <one-explicit-apk-path> [--abi arm64-v8a|x86_64]');
  }
  const selected = selectedAbi(args.length === 1 ? DEFAULT_ANDROID_ABI : args[2]);
  const value = args[0];
  ...
  return { apk: posix.resolve(cwd, value), abi: selected.abi };
}

export function apkPathFromArguments(args, cwd = process.cwd(), platform = process.platform) {
  return apkSelectionFromArguments(args, cwd, platform).apk;
}
...
export function readUnzipOutput(apk, member = null, { spawn = spawnSync, environment = process.env, abi = DEFAULT_ANDROID_ABI } = {}) {
  ...
  if (member !== null && !required.includes(member)) reject('Only fixed required library members may be streamed.');
...
export async function inspectApk(path, abi = DEFAULT_ANDROID_ABI) {
  const selected = selectedAbi(abi);
  const apk = apkPathFromArguments([path]);
  const before = apkMetadata(apk);
  const sha256 = await apkHash(apk, before.size);
  const archive = inspectArchiveListing(readUnzipOutput(apk, null, { abi: selected.abi }), selected.abi);
  const libraries = [];
  for (const member of expectedLibrariesForAbi(selected.abi)) {
    const bytes = readUnzipOutput(apk, member, { abi: selected.abi });
    libraries.push({ member, sha256: createHash('sha256').update(bytes).digest('hex'), elf: checkElf64Android(bytes, selected.abi) });
  }
  ...
  return {
    schemaVersion: 1, scope: 'passive APK selected-library structure only', selectedAbi: selected.abi,
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const { apk, abi } = apkSelectionFromArguments(process.argv.slice(2));
    process.stdout.write(`${JSON.stringify(await inspectApk(apk, abi), null, 2)}\n`);
  } catch (error) {
