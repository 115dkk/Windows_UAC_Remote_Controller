...
describe('display-only connection continuity', () => {
  ...
  it('coalesces one short loss without resetting the bounded loss deadline', () => {
    vi.useFakeTimers();
    ...
    hook.rerender({ value: false });
    act(() => vi.advanceTimersByTime(CONNECTION_DISPLAY_GRACE_MS - 5001));
    expect(hook.result.current).toBe(true);
    act(() => vi.advanceTimersByTime(1));
    expect(hook.result.current).toBe(false);
  });
  it('keeps a recovered link steady and cancels the old loss timer', () => {
    vi.useFakeTimers();
    const hook = renderHook(({ value }) => useConnectionDisplay(value, 'peer:1'), { initialProps: { value: true } });
    hook.rerender({ value: false });
    act(() => vi.advanceTimersByTime(5000));
    hook.rerender({ value: true });
    act(() => vi.advanceTimersByTime(CONNECTION_DISPLAY_GRACE_MS));
    expect(hook.result.current).toBe(true);
  });
  it('never invents initial connectivity and immediately invalidates unknown, new identity or stopped owner', () => {
    ...
    const hook = renderHook(({ value, identity, active }: Input) => useConnectionDisplay(value, identity, active),
      { initialProps: { value: false, identity: 'peer:1', active: true } as Input });
    expect(hook.result.current).toBe(false);
