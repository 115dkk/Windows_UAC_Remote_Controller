...
pub unsafe fn onActivityCreate(
  ...
  // Only this real registration mints a lease, retaining original GlobalRefs.
  // A live ID collision cannot overwrite another physical instance.
  let registered = match super::activity::register(
    &mut env,
    activity_id,
    activity_name.clone(),
    activity.clone(),
    ...
    Err(_) => {
      log::error!("tao: exact Activity registration unavailable");
      return;
    }
  };
  WINDOW_MANAGER
  ...
  CONTEXTS.lock().unwrap().insert(
    ...
    AndroidContext {
      ...
      activity_name,
      window_created: registered.claimed(),
    },
  );
  let looper = ThreadLooper::for_thread().unwrap();
  setup(PACKAGE.get().unwrap(), thread_env, &looper, activity);
  super::activity::registered(&registered);
...
pub unsafe fn onWindowFocusChanged(
  ...
  let activity_id = jni_call_method!(env, &activity, "getId", "()I", i).unwrap();
  let Some(window_id) = super::activity::original_window_id(&env, activity_id, &activity) else {
    return;
  };
  let event = Event::WindowEvent {
    id: WindowId(super::WindowId(window_id)),
...
pub unsafe fn onActivityDestroy(mut env: JNIEnv, _: JClass, activity: JObject) {
  ...
  // Invalidate before deferred destruction, and ignore stale callback objects.
  let Some(retired) =
    super::activity::retire(&mut env, activity_id, &activity, is_changing_configurations)
  else {
    return;
  };
  ...
  if !is_changing_configurations {
    ...
    if contexts
      .get(&activity_id)
      .is_some_and(|context| retired.matches_context(context.context_jobject))
    {
      contexts.remove(&activity_id);
    }
    drop(contexts);
    let mut managers = WINDOW_MANAGER.lock().unwrap();
    if managers
      .get(&activity_id)
      .is_some_and(|manager| retired.matches_window_manager(&env, manager.as_obj()))
    {
      managers.remove(&activity_id);
    }
    drop(managers);
    wake(Event::WindowEvent {
      id: WindowId(super::WindowId(retired.window_id())),
