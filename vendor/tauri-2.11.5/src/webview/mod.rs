...
pub struct InvokeRequest {
  /// Native-only physical origin. Never taken from body or headers.
  #[cfg(all(target_os = "android", feature = "wry"))]
  pub android_origin: Option<crate::ipc::AndroidInvokeOrigin>,
  /// The invoke command.
  pub cmd: String,
...
```
  ...
  pub fn on_message(self, request: InvokeRequest, responder: Box<OwnedInvokeResponder<R>>) {
    ...
    #[cfg(all(target_os = "android", feature = "wry"))]
    let message = message.with_android_origin(request.android_origin);
    ...
    if let Some((plugin, command_name)) = plugin_command {
      ...
      #[cfg(mobile)]
      {
        if !handled {
          ...
          let reply = move |response: std::result::Result<serde_json::Value, serde_json::Value>| {
            match response {
          ...
          let result = {
            let _ = app_handle;
            match message.android_origin.as_ref() {
              Some(origin) => crate::plugin::mobile::run_command_from_origin(
                plugin,
                origin,
              ...
              None => Err(crate::plugin::mobile::PluginInvokeError::UnreachableWebview),
            }
          };
          #[cfg(not(all(target_os = "android", feature = "wry")))]
          let result = crate::plugin::mobile::run_command(
            plugin,
            &app_handle,
            heck::AsLowerCamelCase(message.command).to_string(),
            payload,
