...
mod authority;
#[cfg(all(target_os = "android", feature = "wry"))]
pub use android_origin::AndroidInvokeOrigin;
...
pub struct InvokeMessage<R: Runtime> {
  #[cfg(all(target_os = "android", feature = "wry"))]
  pub(crate) android_origin: Option<AndroidInvokeOrigin>,
...
impl<R: Runtime> Clone for InvokeMessage<R> {
  fn clone(&self) -> Self {
    Self {
      #[cfg(all(target_os = "android", feature = "wry"))]
      android_origin: self.android_origin.clone(),
      webview: self.webview.clone(),
...
impl<R: Runtime> InvokeMessage<R> {
  ...
  #[cfg(all(target_os = "android", feature = "wry"))]
  pub(crate) fn with_android_origin(mut self, origin: Option<AndroidInvokeOrigin>) -> Self {
    self.android_origin = origin;
    self
  }
