...
impl<'a, M: Manager<crate::Wry>> WebviewWindowBuilder<'a, crate::Wry, M> {
  /// Attach the fixed window to one original framework-registered Activity.
  /// Must be built on the running Tauri runtime thread. This never starts an
  /// Activity or falls back to a different instance; clones share one claim.
  pub fn with_existing_android_activity(
    mut self,
    activity: crate::tao::platform::android::ExistingActivity,
  ) -> Self {
