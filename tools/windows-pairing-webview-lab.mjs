...
export async function provePairingLaunch({ page, ps, profile, evidence, confirmService, clientPid }) {
  const consent = () => JSON.parse(ps(`$ErrorActionPreference='Stop';$session=(Get-Process -Id ${clientPid}).SessionId;$rows=@(Get-Process -Name consent -ErrorAction SilentlyContinue | Where-Object SessionId -eq $session);if($rows.Count -eq 0){'null';exit};if($rows.Count -ne 1){throw 'Ambiguous consent process'};$p=$rows[0];$image=(Get-CimInstance Win32_Process -Filter ('ProcessId='+$p.Id)).ExecutablePath;if($image -ne (Join-Path $env:WINDIR 'System32/consent.exe')){throw 'Unexpected consent image'};@{pid=$p.Id;created=$p.StartTime.ToUniversalTime().Ticks.ToString()}|ConvertTo-Json -Compress`));
  ...
  try {
    ...
    try {
      ...
      if (prompt) {
        ps(`$ErrorActionPreference='Stop';$p=Get-Process -Id ${prompt.pid} -ErrorAction SilentlyContinue;if($p){$held=$p.Handle;$image=(Get-CimInstance Win32_Process -Filter ('ProcessId='+$p.Id)).ExecutablePath;if($p.StartTime.ToUniversalTime().Ticks.ToString() -ne '${prompt.created}' -or $image -ne (Join-Path $env:WINDIR 'System32/consent.exe')){throw 'Consent identity changed'};Stop-Process -InputObject $p -Force;$p.Dispose()}`);
      }
    }
  }
}
