...
- 사용자 지시(2026-09-24): PR 머리 커밋의 검사가 모두 통과하면 사용자에게 다시
  묻지 않고 merge한다. main에 merge하면 main-release.yml이 다음 버전(fix·docs는
  patch, feat는 minor)을 스스로 게시하므로 merge가 곧 릴리스 발행이다. 검사가
  하나라도 실패했거나 아직 돌고 있으면 merge하지 않는다. 문서만 merge해도
  릴리스가 나가므로 문서 변경은 진행 중인 제품 PR에 함께 싣는다.
