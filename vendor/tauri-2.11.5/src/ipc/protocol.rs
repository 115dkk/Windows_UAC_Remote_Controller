      ...
      "POST" => {
        if let Some(webview) = manager.get_webview(label) {
          match parse_invoke_request(&manager, request) {
            Ok(request) => {
              #[cfg(feature = "tracing")]
              let request_span = tracing::trace_span!("ipc::request::handle");
              ...
              webview.on_message(
                ...
                Box::new(move |_webview, _cmd, response, _callback, _error| {
                  ...
                  let response_span = match &response {
                    InvokeResponse::Ok(InvokeResponseBody::Json(_)) => tracing::trace_span!(
                      "ipc::request::response",
                      kind = "json",
                      mime_type = tracing::field::Empty
                    )
                    .entered(),
                    InvokeResponse::Ok(InvokeResponseBody::Raw(_)) => tracing::trace_span!(
                      "ipc::request::response",
                      kind = "raw",
                      mime_type = tracing::field::Empty
                    )
                    .entered(),
                    InvokeResponse::Err(_) => tracing::trace_span!(
                      "ipc::request::response",
                      kind = "error",
                      mime_type = tracing::field::Empty
                    )
...
fn handle_ipc_message<R: Runtime>(request: Request<String>, manager: &AppManager<R>, label: &str) {
  ...
  let Some(android_origin) = crate::ipc::AndroidInvokeOrigin::from_extensions(request.extensions()) else {
    return;
  };
  let Some(webview) = manager.get_webview(label) else {
    return;
  };
  ...
  match message {
    Ok(message) => {
      ...
      let request = InvokeRequest {
        #[cfg(all(target_os = "android", feature = "wry"))]
        android_origin: Some(android_origin.clone()),
        cmd: message.cmd,
        callback: message.callback,
        error: message.error,
      ...
      webview.on_message(
        ...
        Box::new(move |webview, cmd, response, callback, error| {
          ...
          fn responder_eval<R: Runtime>(
            ...
            {
              let _ = webview;
              let _ = origin.0.eval(eval_js);
            }
            #[cfg(not(all(target_os = "android", feature = "wry")))]
            let _ = webview.eval(eval_js);
          }

          let can_use_channel_for_response = cmd != crate::ipc::channel::FETCH_CHANNEL_DATA_COMMAND
            && !options.custom_protocol_ipc_blocked
            // Channels retain logical labels. A physical-origin response uses
            // the original JNI WebView instead, even across logical-ID reuse.
            && !cfg!(all(target_os = "android", feature = "wry"));
    ...
    Err(_) => {
      ...
      #[cfg(all(target_os = "android", feature = "wry"))]
      let _ = android_origin.0.eval(script);
      #[cfg(not(all(target_os = "android", feature = "wry")))]
...
fn parse_invoke_request<R: Runtime>(
  ...
  #[cfg(all(target_os = "android", feature = "wry"))]
  let android_origin = Some(
    crate::ipc::AndroidInvokeOrigin::from_extensions(request.extensions())
      .ok_or("originating Android view is unavailable")?,
  );
  #[allow(unused_mut)]
