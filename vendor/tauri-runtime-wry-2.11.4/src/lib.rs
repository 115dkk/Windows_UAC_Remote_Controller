...
impl WindowIdStore {
  ...
  #[cfg(target_os = "android")]
  fn remove_exact(&self, w: &TaoWindowId, expected: WindowId) {
    let mut entries = self.0.lock().unwrap();
    if entries.get(w) == Some(&expected) {
      entries.remove(w);
    }
  }
}
...
impl<T: UserEvent> Context<T> {
  fn create_window<F: Fn(RawWindow) + Send + 'static>(
    ...
    // Exact Android attachment is admitted on the retained runtime thread.
    // Report synchronous Tao/Wry construction failure instead of publishing a
    // phantom logical window. Other platforms retain their original behavior.
    #[cfg(target_os = "android")]
    let exact_attachment = pending.window_builder.exact_android_activity;
    #[cfg(not(target_os = "android"))]
    let exact_attachment = false;
    if exact_attachment && current_thread().id() != self.main_thread_id {
    ...
    send_user_message(
      ...
      Message::CreateWindow(
        ...
        exact_attachment.then_some(completion_tx),
      ),
    )?;
    if exact_attachment {
      // send_user_message executes synchronously on this exact runtime thread;
      // never block Android main or wait for its asynchronous physical view.
      completion_rx
        .try_recv()
...
impl WindowBuilderWrapper {
  /// Select an original framework lease, never a caller ID/class or new Activity.
  pub fn with_existing_android_activity(
    mut self,
    activity: tao::platform::android::ExistingActivity,
  ) -> Self {
    self.inner = self.inner.with_existing_activity(activity);
    self.exact_android_activity = true;
    self
  }
}
...
pub enum Message<T: 'static> {
  ...
  CreateWindow(WindowId, CreateWindowClosure<T>, Option<Sender<Result<()>>>),
  CreateRawWindow(
    WindowId,
...
fn handle_user_message<T: UserEvent>(
  ...
  match message {
    ...
    Message::CreateWindow(window_id, handler, completion) => match handler(event_loop) {
      Ok(webview) => {
...
fn handle_event_loop<T: UserEvent>(
  ...
  match event {
    ...
    Event::WindowEvent {
      ...
      #[cfg(target_os = "android")]
      let original_tao_window_id = window_id;
      if let Some(window_id) = window_id_map.get(&window_id) {
        ...
        match event {
          ...
          TaoWindowEvent::Destroyed => {
            #[cfg(target_os = "android")]
            window_id_map.remove_exact(&original_tao_window_id, window_id);
...
fn create_window<T: UserEvent, F: Fn(RawWindow) + Send + 'static>(
  ...
  if let Some(webview) = webview {
    ...
    match created {
      ...
      Err(error) => {
        #[cfg(target_os = "android")]
        context.window_id_map.remove_exact(&window.id(), window_id);
        return Err(error);
      }
    }
  }
...
"#,
  ...
  #[cfg(target_os = "android")]
  {
    // Bind Wry's independent physical origin through the ORIGINAL GlobalRef
    // held by this Tao Window; never recapture by reusable ID/current proxy.
    let activity = window.existing_activity();
    let origin = activity
      .with_original_activity(|env, original| wry::android_activity_origin(env, original))
      .map_err(|error| Error::CreateWebview(Box::new(error)))?
      .map_err(|error| Error::CreateWebview(Box::new(error)))?;
    webview_builder = webview_builder.with_android_activity_origin(origin);
