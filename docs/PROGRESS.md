...
2026-09-19 · **관리자 동의형 UAC의 원격 승인이 실제 PC와 휴대폰에서 끝까지 동작했다. 제품 전체가 완성된 상태는 아니다.**

## 9월 24일: 집 밖에서 연결하기

- 9월 22일에 들어간 외부 연결 기능은 실제로는 어느 공유기에서도 외부 주소를 만들지
  못했다. 포트 매핑을 PCP로만 요청했는데 가정용 공유기는 대개 PCP를 지원하지 않고,
  UPnP는 조회만 하고 결과를 버렸으며 그나마 v2 장비만 알아봤다. 이제 UPnP v1과 v2로
  실제 매핑을 만들고, 다른 기기의 매핑은 덮어쓰지 않는다.
- 사용자의 공유기(LG U+ GAPM-7500)는 PCP도 UPnP도 제공하지 않는다. 그래서 외부 주소를
  얻는 방법을 셋으로 늘렸다. 자동 매핑, 사용자가 공유기에서 연 포트(공인 IP는 STUN으로
  확인), 직접 입력이다. 네트워크 코드는 `direct-network` 크레이트로 옮겼고, PC 앱에
  `외부 연결` 탭을 새로 만들었다.
- 포트포워딩(TCP 7443)을 건 뒤 LTE 휴대폰에서 PC까지 연결되는 것을 실제로 확인했다.
  다만 Wi-Fi를 끄고 LTE로 다시 붙기까지 292초가 걸렸다. 원인은 셋이었다. 끊긴 Wi-Fi
  연결이 중계의 방을 5분 무활동 제한까지 차지했고, 휴대폰은 LTE에서 닿지 않는 LAN
  주소 연결을 매번 5초씩 기다렸으며, 설정을 바꿔도 휴대폰은 몇 분 뒤에야 새 주소를 물었다.
  1.4.0은 새 입장이 오면 죽은 쌍을 끊고, 휴대폰이 주소들에 겹쳐 연결하며, 옛 주소만 가진
  휴대폰의 세션을 한 번 끊어 곧바로 다시 묻게 한다. 1.4.0을 실제 기기에 설치해 재 보니 Wi-Fi를
  끈 뒤 LTE 세션까지 약 1초, Wi-Fi를 켠 뒤 집 LAN 세션까지 0.9초였다. LTE에서의 UAC 승인은
  아직 재지 않았다.
- 승인이나 거부를 누르면 요청 카드가 사라질 뿐 결과를 알 수 없었다. 네이티브는 단계와 PC의
  서명된 결과를 이미 넘기고 있었는데 화면이 읽지 않았다. 1.5.0은 누른 자리에서 단계와 결과를
  보여 주고, 연결이 끊기면 방화벽 안내 대신 다시 연결하는 중을, 오래 끊기면 실패 종류에 맞는
  안내를 보여 준다.
- 집 밖 네트워크에 있는 휴대폰에서 1.5.1 설치 확인 창을 승인했다. 첫 승인은 PC가 적용하지
  않았다. 창이 뜬 지 1.2초 만에 읽은 내용을 휴대폰에 보냈는데 그 뒤 창에 요소 하나가 더 붙어,
  적용 직전에 다시 읽은 창과 비교가 어긋났기 때문이다(PC 이벤트 로그의 창 구조 기록으로 확인).
  1.5.2는 창 내용이 1초 동안 그대로일 때 보내고, 그래도 거부되면 5초를 기다리지 않고 같은
  요청을 다시 보내며, 휴대폰은 그 사실을 알린다. 단위 테스트가 실제 이벤트 로그와 진단 파일에
  가짜 기록을 쓰던 문제도 막았다.
- 설치할 때마다 서비스가 다시 시작되면 휴대폰이 집 Wi-Fi에서도 10~13초 뒤에야 다시 붙었다.
  휴대폰은 끊긴 직후 한 번 다시 걸었지만 그때는 서비스가 멈춰 있었고, 그 뒤 재시도를 깨우는
  곳이 없어 15초 주기 점검을 기다렸다. 1.5.3은 끊긴 뒤 첫 시도를 2분 동안 1초 간격으로 이어
  가고, PC는 외부 주소를 찾는 중에 온 주소 질문을 보관했다가 답해 방금 붙은 연결을 끊지 않는다.
  1.5.3끼리 재 보니 서비스가 Running이 된 뒤 0.36초 만에 다시 붙었고, 그 연결은 5분 넘게
  유지됐다.
- 요청의 세부 내용을 펼쳐 두면 휴대폰 시계의 분이 바뀌는 순간 '세부 내용을 확인할 수
  없습니다'로 바뀌었다. 세부 내용은 알림 시간 규칙 때문에 그 분 안에서만 유효한데, 화면이
  만료를 실패처럼 보여 준 것이다. 이제는 만료되면 스스로 다시 읽는다.

## 9월 25일: 정전 뒤 서비스가 다시 서지 않았다

- 9월 25일 10:31 PC 전원이 갑자기 끊겼다가 켜진 뒤, 서비스가 시작할 때마다 0.2초 만에 끝나
  30초 간격으로 440번 넘게 다시 시작했다. 휴대폰에는 'PC는 응답했지만 휴대폰 승인이 연결을
  받지 않습니다'가 떴다. 서비스는 '시작됨'을 기록하고 내장 중계를 연 직후, UAC 감시 프로세스를
  띄우는 단계에서 끝났다(평소 이 단계는 시작 33ms 뒤 '감시 시작'을 남긴다). 남는 기록은
  '코드 1'뿐이라 감시 단계 안의 어느 검사가 거부했는지는 아직 모른다.
- 1.5.5는 감시 프로세스를 띄우지 못해도 서비스를 끝내지 않는다. 1초에서 60초까지 간격을
  늘려 가며 다시 시도하고, 그동안 휴대폰 연결은 유지된다. 거부 사유(고정된 오류 종류와 단계,
  HRESULT)를 진단 파일에 `watcher_start_failed`로, 시작 뒤 서비스가 끝난 단계는
  `runtime_failure`로 남긴다.

## 9월 19일: 실기 승인 성공과 그때까지 걷어낸 결함들

- 실제 PC에 뜬 Windows 동의 창을 연결된 휴대폰에서 지문으로 승인했고, 요청한 프로그램이
  관리자 권한으로 실행됐다. 승격 여부는 `whoami /groups`가 돌려준
  `Mandatory Label\High Mandatory Level`과 `BUILTIN\Administrators`로 확인했다. 확인한 것은
  관리자 동의형 한 가지다. 자격 증명형과 Windows Hello형은 그대로 지원하지 않는다.
- 첫 실제 요청이 휴대폰 쪽 처리를 통째로 죽이던 사슬을 끊었다. 동의 창은 파일 위치를 늘
  보여 주지는 않고 규격도 그것을 정상으로 규정하는데, 휴대폰이 그 값을 잘못된 요청으로 보고
  알림 만들기를 거부했으며 그 거부가 승인 소유자의 종료까지 번졌다. 거부의 이유를 지우던
  `catch (_: Exception)` 때문에 진단 다섯 판을 태우고서야 자리를 찾았다.
- 승인 단추가 회색으로 굳는 증상이 두 번 나왔고 원인은 서로 달랐다. 한 번은 끝난 승인의
  뒷정리가 실패했을 때 그 자리를 놓아 주는 재시도가 앱을 내릴 때만 돌던 것이고, 한 번은 그
  재시도 바로 위에 넣은 조기 반환이 그것을 건너뛰게 만든 것이다. 뒤엣것은 앞엣것의 수정을
  한 줄 위에서 무효로 만든 회귀였다. 지금은 그 함수를 빠져나가는 길이 하나뿐이라는 것을
  `tools/android-approval-slot-contract.test.mjs`가 소스에서 검사한다.
- 윈도우가 휴대폰의 서명된 결정을 적용하지 않고 거절할 때, 이유를 알면서도 버리고 있었다.
  이제 `UAC_APPLY_V1 ... refused=...` 한 줄로 이벤트 로그에 남고 `tools/read-prompt-events.ps1`로
  읽는다. 동의 창의 구조도 같은 방식으로 남긴다. 창에 적힌 글자는 어느 쪽에도 남기지 않는다.
- 요청 카드가 떴다가 사라지고 다시 나타나던 것을 고쳤다. 표시 기한은 벽시계 분 단위로 잘리므로
  58초에 도착한 요청은 2초짜리 기한을 받는다. 기한이 끝나면 화면이 먼저 내용을 거두고 그다음에
  새 상태를 물으러 갔기 때문에, 그 왕복 동안 빈 화면이 보였다. 이제 기한보다 먼저 묻고 기한에
  거둔다. 표시 가능한 시간은 1밀리초도 늘지 않는다.
- 남은 것: 승인 창을 펼쳐 명령줄을 따로 보여 주는 일, PC 앱의 `활동 기록 사용 불가`와 휴대폰
  아래쪽의 `연결된 PC / 사용 불가`, 요청이 끝난 뒤에도 화면에 남는 동의 창, 실험실의 간헐 실패
  두 건. 최종 보안 감사와 구조 리팩터링도 그대로 남아 있다.
