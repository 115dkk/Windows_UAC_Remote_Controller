...
pub(crate) fn configure_relay(endpoint: std::net::SocketAddr) -> Result<(), ServiceError> {
    configure_relay_mode(Some(endpoint))
}

pub(crate) fn configure_relay_mode(
    ...
    if status.state == Some(ServiceState::Running) {
        return crate::management_mutation(endpoint.map_or(
            crate::management_protocol::ManagementRequest::UseEmbeddedRelay,
            |address| crate::management_protocol::ManagementRequest::SetRelay { address },
        ));
    }
...
pub(crate) fn mutate(command: Command) -> Result<ServiceSnapshot, ServiceError> {
    ...
    match command {
        ...
        Command::Uninstall => {
            stop(&service, began)?;
            ffi::remove_embedded_relay_firewall()?;
            service
                .delete()
