    ...
    F: FnMut(event::Event<'_, T>, &event_loop::EventLoopWindowTarget<T>, &mut ControlFlow),
  {
    ...
    'event_loop: loop {
      ...
      if let Some(window_id) = resized_window_id {
        if self.running {
          let size = activity::geometry_target_for_window(window_id.0 .0)
            .map(|target| geometry_size(&target))
...
impl<T: 'static> EventLoopWindowTarget<T> {
  pub fn primary_monitor(&self) -> Option<monitor::MonitorHandle> {
    // Android has no process-wide primary Activity. Do not select an arbitrary
    // registration when more than one logical window is live.
    let mut targets = activity::live_geometry_targets().into_iter();
    let target = targets.next()?;
    if targets.next().is_some() {
      return None;
    }
    Some(monitor::MonitorHandle {
      inner: MonitorHandle { target },
    })
  }
  ...
  pub fn available_monitors(&self) -> VecDeque<MonitorHandle> {
    activity::live_geometry_targets()
      .into_iter()
      .map(|target| MonitorHandle { target })
      .collect()
  }
...
pub struct PlatformSpecificWindowBuilderAttributes {
  ...
  pub existing_activity: Option<activity::ExistingActivity>,
...
pub struct Window {
  ...
  // Owns the original GlobalRefs for the lifetime of raw handles borrowed from
  // this Window. Configuration recreation never rebinds this physical lease.
  activity: activity::ExistingActivity,
}
...
impl Window {
  pub fn new<T: 'static>(
    ...
    let activity = if let Some(activity) = pl_attrs.existing_activity {
      if pl_attrs.activity_name.is_some() {
        return Err(os_error!(OsError::NoAvailableActivity));
      }
      activity
        .claim()
        .map_err(|_| os_error!(OsError::NoAvailableActivity))?;
      activity
    } else {
      match pl_attrs.activity_name {
        Some(activity_name) => {
          ...
          activity::claim_id(activity_id).map_err(|_| os_error!(OsError::NoAvailableActivity))?
        }
        None => activity::claim_next().map_err(|_| os_error!(OsError::NoAvailableActivity))?,
      }
    };
    let activity_id = activity.id();
    let activity_name = activity.activity_name().to_owned();
    if let Some(context) = ndk_glue::CONTEXTS.lock().unwrap().get_mut(&activity_id) {
      if activity.matches_context(context.context_jobject) {
  ...
  pub fn primary_monitor(&self) -> Option<monitor::MonitorHandle> {
    Some(monitor::MonitorHandle {
      inner: MonitorHandle {
        target: self.activity.geometry_target(),
      },
    })
  }
  ...
  pub fn outer_size(&self) -> PhysicalSize<u32> {
    geometry_size(&self.activity.geometry_target())
  }
  ...
  pub fn raw_window_handle_rwh_04(&self) -> rwh_04::RawWindowHandle {
    ...
    if let Some(w) = self.activity.window_manager() {
  ...
  pub fn raw_window_handle_rwh_06(&self) -> Result<rwh_06::RawWindowHandle, rwh_06::HandleError> {
    // TODO: Use main activity instead?
    if let Some(w) = self.activity.window_manager() {
      // The original GlobalRef remains owned by self for this borrowed handle.
      let native_window = std::ptr::NonNull::new(w.as_obj().as_raw() as *mut _)
  ...
  pub fn existing_activity(&self) -> activity::ExistingActivity {
    self.activity.clone()
  }
...
pub struct MonitorHandle {
  target: activity::GeometryTarget,
}
...
fn geometry_size(target: &activity::GeometryTarget) -> PhysicalSize<u32> {
  target
    .with_window_manager(|env, window_manager| {
      let sdk_int = env
        .get_static_field("android/os/Build$VERSION", "SDK_INT", "I")
        .and_then(|value| value.i())
        .ok()?;
      let size = if sdk_int >= 30 {
        current_window_metrics_size(env, window_manager)
      } else {
        legacy_display_metrics_size(env, window_manager)
      };
      if size.is_none() && env.exception_check().unwrap_or(false) {
...
fn configuration_scale_factor() -> f64 {
...
impl MonitorHandle {
  ...
  pub fn size(&self) -> PhysicalSize<u32> {
    // The gap between physical incarnations is unavailable, never a fallback
    // to another Activity. Geometry queries preserve this monitor's logical Arc.
    geometry_size(&self.target)
  }
