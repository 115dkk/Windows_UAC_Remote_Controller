...
Fifteen rules separate `CreatePc` from repeated trusted device enrollment, allowing multiple approvers under the **same PC**. Each device has linear `CurrentRegistry(pc, device, revision)`/`RetiredRegistry(pc, device, revision)` state, while its unchanged public purpose keys reside in persistent `EnrollmentKeys`. A request's `Building(pc, binding)` phase captures immutable per-device `Snapshot` facts containing the exact current revision and purpose keys. Publish consumes Building and creates exactly one **global `Pending(pc, binding)`**. Every eligible device and both decision purposes must consume that same Pending; there is no per-device replay guard.
...
`EnrollmentKeys(pc, device, approval_key, denial_key)` is produced **only by the existing TrustedEnrollment rule**, from the same fresh keys already placed in PhoneKeys and public output. CurrentRegistry and RetiredRegistry retain only PC/device/revision; capture, acceptance and same-key re-enrollment/replacement join the immutable keys when needed. The frozen Snapshot still carries the complete keys, revision and binding. This is not persistent membership: revocation still removes CurrentRegistry, and the surviving key fact cannot authorize an acceptance without that exact current revision and global Pending.

`RequestOrigin(pc, binding)` is produced **only by OpenActualRequest**, alongside Building. It is a redundant premise on capture, publication, both acceptance rules, cancellation and expiry. Every corresponding old Building/Pending/Snapshot already descends from that same open, and the additional fact is never consumed. It does not replace Building, Snapshot or Pending, publish binding data early, or allow capture after publication. UserAuthenticatesExactApproval and both signing rules remain unchanged and receive the same attacker-controlled public traffic; no origin predicate filters their inputs.

The intended trace-equivalence is to the **current overapproximating model**, not a new implementation proof: reattach the unique EnrollmentKeys tuple to each three-field current/retired fact to recover the former five-field state; erase the two derived persistent fact kinds. Conversely, every old enrollment/open can produce those facts, and their new premises were already implied by the old reachable states. The original theory already keeps each fresh device's keys unchanged across revisions; splitting them out does not introduce key rotation or prove arbitrary same-device key replacement. All nine lemmas, action facts, public messages, signature checks and canary marker spans are retained. No sources/reuse axiom, assumed conclusion, trace restriction or device/session bound was added.
